...
use crate::{Direction, GenericBPlusTree, Node};
...
impl<'t, K: Clone + Ord, V, const IC: usize, const LC: usize> RawSharedIter<'t, K, V, IC, LC> {
            ...
            if let Some((parent_guard, p_cursor)) = self.parent.as_ref() {
                ...
                if let Some(pos) = bounded_pos {
                    ...
                    let opt =
                        match self
                            .tree
                            .find_nearest_leaf(parent_guard, direction, &self.eg)?
                        {
                            // TODO check if this is garanteed to return a sibling leaf
        ...
        Q: ?Sized + Ord,
    {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                let (guard, parent_opt) = self
                    .tree
                    .find_shared_leaf_and_optimistic_parent(key, &self.eg);
                (
                    Self::leaf_lt(guard),
    ...
    pub fn seek_to_first(&mut self) {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                ...
                let (guard, parent_opt) = self
                    .tree
                    .find_first_shared_leaf_and_optimistic_parent(&self.eg);
                (
                    Self::leaf_lt(guard),
                    parent_opt.map(|(parent, pos)| (Self::parent_lt(parent), pos)),
                )
    ...
    pub fn seek_to_last(&mut self) {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                ...
                let (guard, parent_opt) = self
                    .tree
                    .find_last_shared_leaf_and_optimistic_parent(&self.eg);
                (
                    Self::leaf_lt(guard),
                    parent_opt.map(|(parent, pos)| (Self::parent_lt(parent), pos)),
                )
        ...
        Q: ?Sized + Ord,
    {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                ...
                let (guard, parent_opt) = self
                    .tree
                    .find_exclusive_leaf_and_optimistic_parent(key, &self.eg);
                (
                    Self::leaf_lt(guard),
                    parent_opt.map(|(parent, pos)| (Self::parent_lt(parent), pos)),
                )
    ...
    pub fn seek_to_first(&mut self) {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                ...
                let (guard, parent_opt) = self
                    .tree
                    .find_first_exclusive_leaf_and_optimistic_parent(&self.eg);
                (
                    Self::leaf_lt(guard),
                    parent_opt.map(|(parent, pos)| (Self::parent_lt(parent), pos)),
                )
    ...
    pub fn seek_to_last(&mut self) {
        let (guard, parent_opt) = match self.leaf.take() {
            ...
            _ => {
                ...
                let (guard, parent_opt) = self
                    .tree
                    .find_last_exclusive_leaf_and_optimistic_parent(&self.eg);
                (
                    Self::leaf_lt(guard),
                    parent_opt.map(|(parent, pos)| (Self::parent_lt(parent), pos)),
                )
...
#[cfg(test)]
mod tests {
    use super::{RawExclusiveIter, RawSharedIter};
    ...
    fn btree_map_sanity_check() {
        ...
        for i in (0..n).rev() {
            assert_eq!(
                iter.remove(&strings[i]).as_ref().map(|(k, v)| (k, v)),
                Some((&strings[i], &data[i]))
            );
        }
