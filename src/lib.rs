...
impl<K: Clone + Ord, V, const IC: usize, const LC: usize> GenericBPlusTree<K, V, IC, LC> {
    ...
    pub(crate) fn find_parent<'t, 'g, 'e>(
        &'t self,
        needle: &impl HybridGuard<Node<K, V, IC, LC>>,
        eg: &'e epoch::Guard,
    ) -> error::Result<ParentHandler<'t, 'e, K, V, IC, LC>>
    where
        K: Ord,
    {
    ...
    pub(crate) fn find_nearest_leaf<'t, 'g, 'e>(
        &'t self,
        ...
        Option<(
            ...
            (OptimisticGuard<'e, Node<K, V, IC, LC>>, u16),
        )>,
    >
    where
        K: Ord,
    ...
    fn find_leaf_and_parent_from_node<'t, 'g, 'e>(
        &'t self,
            ...
            let perform = || {
                ...
                let leaf_guard = loop {
                    ...
                    if (level + 1) as usize == self.height.load(Ordering::Acquire) {
                        ...
                        let guard =
                            GenericBPlusTree::lock_coupling_shared(&target_guard, c_swip, eg)?;
                        p_guard = Some((target_guard, pos));
            ...
            let perform = || {
                ...
                let leaf_guard = loop {
                    ...
                    if (level + 1) as usize == self.height.load(Ordering::Acquire) {
                        ...
                        let guard =
                            GenericBPlusTree::lock_coupling_exclusive(&target_guard, c_swip, eg)?;
                        p_guard = Some((target_guard, pos));
        ...
        Q: ?Sized + Ord,
    {
        ...
        let opt = if let Some(((mut guard, pos), _parent_opt)) =
                ...
                match root_guard_x.as_mut() {
                    Node::Internal(root_internal_node) => {
...

                        {
                            ...
                            new_root
                                .insert(split_key, old_root_edge)
        ...
        K: Ord,
    {
        ...
        match parent_handler {
            ...
            ParentHandler::Parent {
                ...
                let parent_merge = || {
                    // TODO test if we should ensure parent merges always happen
...
impl<K: fmt::Debug, V: fmt::Debug, const LEAF_CAPACITY: usize> fmt::Debug
    for LeafNode<K, V, LEAF_CAPACITY>
{
...
impl<K, V, const LEAF_CAPACITY: usize> LeafNode<K, V, LEAF_CAPACITY> {
    ...
    pub(crate) fn kv_at_mut(&mut self, pos: u16) -> error::Result<(&K, &mut V)> {
        Ok(unsafe {
            (
                self.keys.get_unchecked(pos as usize),
                self.values.get_unchecked_mut(pos as usize),
            )
        })
    }
    ...
    {
        match (
            self.lower_fence().map(Borrow::borrow),
            self.upper_fence().map(Borrow::borrow),
        ) {
...
impl<K: Clone, V, const LEAF_CAPACITY: usize> LeafNode<K, V, LEAF_CAPACITY> {
    pub(crate) fn split(&mut self, right: &mut LeafNode<K, V, LEAF_CAPACITY>, split_pos: u16) {
        ...
        right
            .keys
            .extend(self.keys.drain((split_pos + 1) as usize..));
        right
            .values
            .extend(self.values.drain((split_pos + 1) as usize..));
    ...
    pub(crate) fn merge(&mut self, right: &mut LeafNode<K, V, LEAF_CAPACITY>) -> bool {
        if (self.len + right.len) as usize > LEAF_CAPACITY {
            // TODO better merge threshold
            return false;
        }
...
impl<K: fmt::Debug, V: fmt::Debug, const INNER_CAPACITY: usize, const LC: usize> fmt::Debug
    for InternalNode<K, V, INNER_CAPACITY, LC>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<K, V, const INNER_CAPACITY: usize, const LC: usize> InternalNode<K, V, INNER_CAPACITY, LC> {
    ...
    pub(crate) fn edge_at(
        &self,
        pos: u16,
    ) -> error::Result<&Atomic<HybridLatch<Node<K, V, INNER_CAPACITY, LC>>>> {
        if pos == self.len {
    ...
    pub(crate) fn insert(
        &mut self,
        key: K,
        value: Atomic<HybridLatch<Node<K, V, INNER_CAPACITY, LC>>>,
    ) -> Option<u16>
    where
        K: Ord,
    {
    ...
    pub(crate) fn remove<Q>(
        &mut self,
        key: &Q,
    ) -> Option<Atomic<HybridLatch<Node<K, V, INNER_CAPACITY, LC>>>>
    where
        K: Borrow<Q> + Ord,
        Q: ?Sized + Ord,
    {
    ...
    pub(crate) fn remove_at(
        &mut self,
        pos: u16,
    ) -> (K, Atomic<HybridLatch<Node<K, V, INNER_CAPACITY, LC>>>) {
        let key = self.keys.remove(pos as usize);
...
impl<K: Clone, V, const INNER_CAPACITY: usize, const LC: usize>
    InternalNode<K, V, INNER_CAPACITY, LC>
{
    pub(crate) fn split(
        &mut self,
        right: &mut InternalNode<K, V, INNER_CAPACITY, LC>,
        split_pos: u16,
    ) {
        let split_key = self.key_at(split_pos).expect("should exist").clone();
        right.lower_fence = Some(split_key.clone());
        ...
        right
            .edges
            .extend(self.edges.drain((split_pos + 1) as usize..));
    ...
    pub(crate) fn merge(&mut self, right: &mut InternalNode<K, V, INNER_CAPACITY, LC>) -> bool {
        ...
        self.keys.push(
            right
                .lower_fence
                .take()
                .expect("right node must have lower fence"),
        );
        self.edges
            .push(left_upper_edge.expect("left node must have upper edge"));
...
#[cfg(test)]
mod tests {
    use super::{Direction, LeafNode, ParentHandler};
    ...
    fn sample_tree_find_parent() {
        ...
        if let ParentHandler::Parent {
            parent_guard,
            pos: _,
        } = bptree.find_parent(&leaf, eg).unwrap()
        {
            assert_eq!(
                parent_guard.as_internal().keys.first(),
                Some(&"0002".to_string())
            );
            ...
            if let ParentHandler::Parent {
                parent_guard,
                pos: _,
            } = bptree.find_parent(&parent_guard, eg).unwrap()
            {
                assert_eq!(
                    parent_guard.as_internal().keys.first(),
                    Some(&"0003".to_string())
                ...
                if let ParentHandler::Root { tree_guard } =
                    bptree.find_parent(&parent_guard, eg).unwrap()
                {
    ...
    fn sample_tree_find_nearest_leaf() {
        ...
        let (next_leaf, (parent, _pos)) = bptree
            .find_nearest_leaf(&leaf, Direction::Forward, eg)
            .unwrap()
            .unwrap();
        ...
        let (next_leaf, (parent, _pos)) = bptree
            .find_nearest_leaf(&next_leaf, Direction::Forward, eg)
            .unwrap()
            .unwrap();
        assert!(next_leaf.is_leaf());
        ...
        let (next_leaf, (parent, _pos)) = bptree
            .find_nearest_leaf(&next_leaf, Direction::Reverse, eg)
            .unwrap()
            .unwrap();
        assert!(next_leaf.is_leaf());
        assert_eq!(next_leaf.keys().first(), Some(&"0003".to_string()));
        ...
        let (next_leaf, (parent, _pos)) = bptree
            .find_nearest_leaf(&parent, Direction::Forward, eg)
            .unwrap()
            .unwrap();
        assert!(next_leaf.is_leaf());
        assert_eq!(next_leaf.keys().first(), Some(&"0005".to_string()));
