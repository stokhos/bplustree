...
use crate::{BPlusTree, InternalNode, LeafNode, Node};
...
fn translate_node(tree_node: TreeNode) -> Atomic<HybridLatch<DefaultNode<String, u64>>> {
    match tree_node {
        TreeNode::Internal {
            ...
            return Atomic::new(HybridLatch::new(Node::Internal(InternalNode {
        ...
        TreeNode::Leaf {
            ...
            return Atomic::new(HybridLatch::new(Node::Leaf(LeafNode {
                len: out_keys.len() as u16,
                keys: out_keys,
                values: out_values,
                lower_fence,
