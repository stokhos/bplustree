...
use crate::latch::{ExclusiveGuard, HybridGuard, HybridLatch, OptimisticGuard, SharedGuard};
...
use registry::{Catalog, DataStructureId, ParentResult};

use self::registry::{CatalogEntry, DataStructureType, ManagedDataStructure, CATALOG_DTID};
...
impl SizeClass {
    ...
    pub fn contains_frame(&self, frame: &HybridLatch<BufferFrame>) -> bool {
        self.frames
            .iter()
            .find(|&f| f as *const _ == frame as *const _)
            .is_some()
    }
    ...
    pub fn allocate_page(
        ...
        dtid: DataStructureId,
        epoch: usize,
    ) -> Result<ExclusiveGuard<'static, BufferFrame>, BufMgrError> {
        let free_bf = self
            .free_frames
            .pop()
    ...
    pub fn random_frame(&'static self) -> &'static HybridLatch<BufferFrame> {
        self.frames
            ...
            .expect("must have at least one frame")
    }
}
...
impl IoSlot {
    ...
    fn new_swizzled(
        bufmgr: &'static BufferManager,
        pid: Pid,
        latched_frame: &'static HybridLatch<BufferFrame>,
    ) -> Self {
        IoSlot {
            bufmgr,
    ...
    fn transition(&mut self, command: IoCommand) -> Result<IoOutcome, BufMgrError> {
        ...
        match (&self.state, command) {
            (Evicted | Aborted, Load) => {
                ...
                let free_bf = size_class
                    .free_frames
                    .pop()
                    .ok_or_else(|| BufMgrError::OutOfFrames(size_class.class))?;
                ...
                self.bufmgr
                    .read_page_sync(self.pid, frame.page)
                    .map_err(|e| {
                ...
                frame.pid = self.pid;
                frame.epoch.store(
                    self.bufmgr.global_epoch.load(Ordering::Acquire),
                    Ordering::Release,
                );
...
impl BufferManager {
            ...
            if let Some(class_size) = 2usize.checked_pow(class as u32) {
                ...
                if n_frames > 0 {
                    ...
                    for frame_idx in 0..n_frames {
                        let ptr =
                            unsafe { (addr as *mut u8).add(frame_idx * class_size) } as *mut Page;
    ...
    pub fn init(&'static self) {
        ...
        let catalog_frame_latch = base_class
            .frames
            .first()
            .expect("base class has zero frames");
        ...
        let read_bytes = self
            .try_read_page_sync(catalog_pid, catalog_frame_x.page)
        ...
        if read_bytes == 0 {
            ...
            catalog_frame_x
                .epoch
                .store(RESERVED_EPOCH, Ordering::Release);

            // Init catalog
            self.catalog.init(self, catalog_frame_x);

            self.catalog
                .store_next_offset(self.offset.load(Ordering::Acquire) as u64);
    ...
    pub fn read_page_sync(&self, pid: Pid, page: &mut Page) -> Result<(), BufMgrError> {
        ...
        let n = pread(&self.fd, &mut slice, offset as i64).map_err(|e| BufMgrError::Io(e))?;
        if n != size {
            println!(
                "BUG pid: {:?}, slice_len: {}, offset: {}, n: {}",
                pid,
                slice.len(),
                offset,
                n
    ...
    pub fn write_page_sync(&self, pid: Pid, page: &Page) -> Result<(), BufMgrError> {
        ...
        let n = pwrite(&self.fd, &slice, offset as i64).map_err(|e| BufMgrError::Io(e))?;
        assert!(n == size);
        Ok(())
    }
    ...
    pub(crate) fn capacity_for<T>(size: usize) -> usize {
        ...
        2usize
            .pow(BASE_SIZE_CLASS as u32)
            .max((size + overhead).next_power_of_two())
    }

    pub fn allocate_page_for<T>(
        &'static self,
        dtid: DataStructureId,
        size: usize,
    ) -> Result<(ExclusiveGuard<'static, BufferFrame>, usize), BufMgrError> {
        let overhead = std::mem::size_of::<Page>() + std::mem::size_of::<T>();
        let class = BASE_SIZE_CLASS.max(
            ((size + overhead).next_power_of_two() as f64)
                .log2()
                .floor() as usize,
        );
        let size_class = self.size_class(class);
        Ok((
            size_class.allocate_page(dtid, self.global_epoch.load(Ordering::Acquire))?,
            size_class.class_size() - overhead,
        ))
    }
        ...
        for size_class in self.classes.iter() {
            for latch in size_class.frames.iter() {
                ...
                if frame.state == BfState::Hot {
                    ...
                    let slice = unsafe {
                        std::slice::from_raw_parts(frame.page as *const _ as *const u8, size)
                    };
                    let n = pwrite(&self.fd, slice, offset as i64).expect("failed to write");
                    assert!(n == size);
                }
            }
        }
    ...
    pub(crate) fn create_data_structure<T, F>(
        &'static self,
        dt_type: DataStructureType,
        f: F,
    ) -> Option<(DataStructureId, Arc<T>)>
    where
        ...
        F: FnOnce(DataStructureId, ExclusiveGuard<'static, T::PageValue, BufferFrame>, usize) -> T,
    {
        ...
        let (guard, capacity) = match self.allocate_page_for::<T::PageValue>(dtid, 1) {
            Ok((bf_guard, capacity)) => {
                let value_guard = ExclusiveGuard::map(bf_guard, |bf| T::bf_to_page_value_mut(bf));
        ...
        self.catalog()
            .update_catalog_entry(CatalogEntry { dtid, dt_type, pid })?;
    ...
    pub fn load_data_structure_root(
        &self,
        dtid: DataStructureId,
        dt_type: DataStructureType,
    ) -> Option<ExclusiveGuard<'static, BufferFrame>> {
    ...
    pub(crate) fn load_data_structure<T, F>(
        &self,
        dtid: DataStructureId,
        dt_type: DataStructureType,
        f: F,
        ...
        F: FnOnce(DataStructureId, ExclusiveGuard<'static, T::PageValue, BufferFrame>) -> T,
    {
        ...
        let value = ExclusiveGuard::map(root_bf, |bf| T::bf_to_page_value_mut(bf));
    ...
    pub(crate) fn evict_data_structure<T: ErasedDataStructure + Send + Sync>(
        &self,
        dtid: DataStructureId,
        dt_type: DataStructureType,
        dt: Arc<T>,
    ) -> Result<(), Arc<T>> {
        if Arc::strong_count(&dt) > 2 {
            // 2 = here and in catalog
            return Err(dt);
        }
    ...
    pub fn evict_data_structure_root(&self, dtid: DataStructureId) -> Option<Pid> {
        ...
        fn evict_frame(
            ...
            dt: &Arc<dyn ErasedDataStructure + Send + Sync>,
            swip: &mut Swip<HybridLatch<BufferFrame>>,
        ) {
            match swip.downcast() {
                RefOrPid::Ref(latch) => {
                    let mut frame_x = latch.exclusive();

                    dt.iterate_children_swips_mut(
                        &mut frame_x,
                        Box::new(|swip| {
                    ...
                    bufmgr
                        .write_page_sync(frame_x.pid, frame_x.page)
...
impl BufferManager {
    ...
    pub fn resolve_swip_fast(
        ...
        match swip_guard.downcast() {
            ...
            RefOrPid::Pid(_) => self.resolve_swip(swip_guard, spin),
        }
    }
    ...
    pub fn resolve_swip(
        ...
        match swip_guard.downcast() {
            ...
            RefOrPid::Pid(pid) => {
                ...
                return self.with_slot_or_create(
                    pid,
                    IoSlot::new_evicted(self, pid),
                    |slot_guard, returned| {
                        match &slot_guard.state {
                            IoState::Evicted | IoState::Aborted => {
                                ...
                                let swip_x_guard = match swip_guard.to_exclusive() {
                                    Ok(guard) => guard,
                                    Err(_) => {
                                        return (false, Err(error::Error::Unwind));
                                        // Retry without cleanup, this can leak a loaded frame
                                    }
                                };
                                if let IoOutcome::SwipAndFrame(swip_x_guard, frame_ref) = slot_guard
                                    .transition(IoCommand::Swizzle(swip_x_guard))
        ...
        F: FnOnce(&mut IoSlot, Option<IoSlot>) -> (bool, R),
    {
        ...
        let mut guard = if existed {
            drop(create_guard);
            returned = Some(
                Arc::try_unwrap(unmoved_slot)
                    .expect("must be the only ref")
                    .into_inner(),
            );
            slot.lock()
        } else {
    ...
    fn try_with_slot_or_create<F, R>(
        &'static self,
        pid: Pid,
        create_slot: IoSlot,
        f: F,
